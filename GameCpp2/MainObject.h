...
public:
  ...
  void Show(SDL_Renderer* des); // Chạy frame
  void HandleInputAction(SDL_Event events, SDL_Renderer* screen); // Xử lí các nút di chuyển 
  void SetMapXY(const int map_x, const int map_y) {map_x_ = map_x, map_y_ = map_y;};

  void DoPlayer(Map& g_map); // Xử lí di chuyển

  void CenterEntityOnMap(Map& g_map);
  void CheckToMap(Map& g_map); // Xử lí va chạm map
...
private:
  ...
  float x_val_; // Giá trị di chuyển
  float y_val_;
  float x_pos_; // Vị trí hiện tại 
  float y_pos_;

  int width_frame_; // Kích thước hình ảnh
  int height_frame_;
};
